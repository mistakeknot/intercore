...
func TestGate_ArtifactExists_SoftFail(t *testing.T) {
	...
	result, err := Advance(ctx, store, id, GateConfig{Priority: 2}, rtStore, nil, nil, nil, nil, nil)
	if err != nil {
		t.Fatalf("Advance: %v", err)
	}
	if !result.Advanced {
...
func TestGate_DisableAll(t *testing.T) {
	...
	result, err := Advance(ctx, store, id, GateConfig{Priority: 0, DisableAll: true}, nil, nil, nil, nil, nil, nil)
	if err != nil {
		t.Fatalf("Advance: %v", err)
	}
	if !result.Advanced {
...
func TestGate_Evidence_Serialized(t *testing.T) {
	...
	_, err := Advance(ctx, store, id, GateConfig{Priority: 0}, rtStore, nil, nil, nil, nil, nil)
	if err != nil {
		t.Fatalf("Advance: %v", err)
	}
...
func TestEvaluateGate_DryRun(t *testing.T) {
	...
	result, err := EvaluateGate(ctx, store, id, GateConfig{Priority: 0}, rtStore, nil, nil, nil, nil)
	if err != nil {
...
func TestGate_DBError(t *testing.T) {
	...
	_, err := EvaluateGate(cancelCtx, store, id, GateConfig{Priority: 0}, rtStore, nil, nil, nil, nil)
	if err == nil {
