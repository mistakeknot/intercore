...
// ConservativeCapConfig returns conservative defaults with slow ramp-up.
// Suitable for agent types with strict rate limits or higher failure rates.
func ConservativeCapConfig() AgentCapConfig {
	return AgentCapConfig{
		MaxConcurrent:     3,                // Conservative max
		RampUpEnabled:     true,             // Start slow
		RampUpInitial:     1,                // Start with 1
...
func DefaultAgentCapsConfig() AgentCapsConfig {
	return AgentCapsConfig{
		...
		PerAgent: map[string]AgentCapConfig{
			"codex":  ConservativeCapConfig(),  // Codex: conservative + ramp-up
			"claude": DefaultAgentCapConfig(),  // Claude: standard caps
		},
	}
}
