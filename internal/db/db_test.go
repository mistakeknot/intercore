...
func TestMigrate_CreatesTablesAndVersion(t *testing.T) {
	...
	// Verify tables exist
	for _, table := range []string{"state", "sentinels", "dispatches", "runs", "phase_events", "run_agents", "run_artifacts", "dispatch_events", "interspect_events", "merge_intents", "coordination_locks", "coordination_events", "run_replay_inputs", "landed_changes", "sessions", "session_attributions"} {
		var name string
...
func TestMigrate_V7ToV8_ArtifactStatus(t *testing.T) {
	...
	if v != 26 {
		t.Fatalf("expected schema version 26, got %d", v)
	}
