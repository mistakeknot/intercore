...
func TestMigrate_CreatesTablesAndVersion(t *testing.T) {
	...
	if v != 25 {
		t.Errorf("SchemaVersion = %d, want 25", v)
	}

	// Verify tables exist
	for _, table := range []string{"state", "sentinels", "dispatches", "runs", "phase_events", "run_agents", "run_artifacts", "dispatch_events", "interspect_events", "merge_intents", "coordination_locks", "coordination_events", "run_replay_inputs", "landed_changes"} {
		var name string
...
func TestMigrate_Concurrent(t *testing.T) {
	...
	if v != 25 {
		t.Errorf("SchemaVersion = %d after concurrent migrate, want 25", v)
	}
}
...
func TestMigrate_V1ToV2(t *testing.T) {
	...
	if v != 25 {
		t.Errorf("SchemaVersion = %d after v1→v7 migrate, want 25", v)
	}
...
func TestMigrate_V2ToV3(t *testing.T) {
	...
	if v != 25 {
		t.Errorf("SchemaVersion = %d after v2→v7 migrate, want 25", v)
	}
...
func TestMigrate_V3ToV4(t *testing.T) {
	...
	if v != 25 {
		t.Errorf("SchemaVersion = %d after v3→v7 migrate, want 25", v)
	}
...
func TestMigrate_V7ToV8_ArtifactStatus(t *testing.T) {
	...
	if v != 25 {
		t.Fatalf("expected schema version 25, got %d", v)
	}
