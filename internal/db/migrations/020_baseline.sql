...
-- v38: first-class L2 agency lifecycle events
...
CREATE INDEX idx_agency_events_agency ON agency_events(agency_name, id);
CREATE INDEX idx_agency_events_run ON agency_events(run_id, id) WHERE run_id IS NOT NULL;
CREATE INDEX idx_agency_events_created ON agency_events(created_at, id);
...
CREATE INDEX idx_audit_log_trace ON audit_log(trace_id) WHERE trace_id != '';
